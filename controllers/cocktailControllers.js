...
const cocktailControllers = {
    ...
    updateCocktail: async(req, res) => {
        try {
            if(req.user.role === 'admin' || req.user.role === 'mod') {
                const cocktail = await Cocktail.findOneAndUpdate({_id: req.params.id}, {...req.body})
    ...
    deleteCocktail: async (req, res) => {
        try {
            if (req.user.role === 'admin' || req.user.role === 'mod') {
                ...
                res.json({ success: true, msg: 'Cocktail was deleted successfully ', cocktailId: deletedCocktail._id })
            } else {
